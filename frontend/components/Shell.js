  ...
  const crumb =
    view === "passport" ? (
      <span className="flex items-center gap-1.5">
        <button onClick={() => goto("assets")} className="hover:underline">
          Assets
        </button>
        <span className="text-ink-faint">›</span>
        <b className="font-semibold text-ink">
          {byId(D.assets, opts.assetId || "ast_p117_n")?.tag} — {byId(D.assets, opts.assetId || "ast_p117_n")?.name}
        </b>
      </span>
    ) : (
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-paper-alt">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main id="main-scroll" className="scrollhide flex-1 overflow-y-auto px-6 py-6">
          {view !== "passport" && VIEW_META[view] && (
            <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
              ...
              {VIEW_META[view].action && (
                <button
                  onClick={() => goto(VIEW_META[view].action.nav)}
          ...
          {view === "passport" && (
            <Passport assetId={opts.assetId || "ast_p117_n"} onCite={setCiteDocId} onOpenDoc={(id) => goto("documents", { docId: id })} onNav={goto} />
          )}
