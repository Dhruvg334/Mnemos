function Box({ x, y, w, h, title, lines = [], tone = "light" }) {
  ...
  return (
    <g>
      ...
      <text x={x + 14} y={y + 23} fill={titleFill} fontSize="12" fontWeight="600">{title}</text>
      {lines.map((line, index) => (
        <text key={line} x={x + 14} y={y + 43 + index * 16} fill={textFill} fontSize="9.5">{line}</text>
      ))}
    </g>
  );
}
...
function Arrow({ x1, y1, x2, y2, dashed = false }) {
  return <path d={`M${x1} ${y1} L${x2} ${y2}`} stroke="#6f7785" strokeWidth="1.3" strokeDasharray={dashed ? "5 5" : undefined} markerEnd="url(#arrow)" />;
}

function Frame({ children, label, legend = [] }) {
  return (
    <figure className="overflow-hidden border-y border-line bg-paper py-5">
      ...
      <div className="overflow-x-auto rounded-2xl border border-line bg-paper-alt p-3">{children}</div>
      {legend.length ? (
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          {legend.map(([title, text], index) => (
            <div key={title} className="grid grid-cols-[26px,1fr] gap-3 text-[12.5px] leading-6 text-ink-soft">
              ...
              <span><strong className="font-semibold text-ink">{title}:</strong> {text}</span>
            </div>
          ))}
        </div>
      ) : null}
...
export function InfrastructureDiagram() {
  ...
  return (
    <Frame label="Production topology" legend={legend}>
      <svg viewBox="0 0 940 410" className="min-w-[760px] w-full" role="img" aria-label="Mnemos production topology">
        <ArrowDefs />
        <Box x={20} y={155} w={150} h={90} title="Browser" lines={["Next.js product UI", "Auth + dashboard", "Technical docs"]} tone="blue" />
        ...
        <Box x={485} y={35} w={180} h={110} title="Agent service" lines={["Orchestration", "Retrieval planning", "Evidence composition", "Safe result contract"]} />
        ...
        <Arrow x1={170} y1={200} x2={235} y2={200} />
        <Arrow x1={410} y1={160} x2={485} y2={95} />
        <Arrow x1={410} y1={240} x2={485} y2={300} />
        <Arrow x1={665} y1={85} x2={745} y2={62} />
        <Arrow x1={665} y1={105} x2={745} y2={165} />
        <Arrow x1={665} y1={285} x2={745} y2={269} />
        <Arrow x1={410} y1={190} x2={745} y2={365} />
      </svg>
    </Frame>
  );
}
...
export function AgentWorkflowDiagram() {
  ...
  return (
    <Frame label="Agentic query workflow" legend={legend}>
      <svg viewBox="0 0 940 220" className="min-w-[760px] w-full" role="img" aria-label="Agentic query workflow">
        <ArrowDefs />
        {nodes.map(([x, title], index) => (
          <g key={title}>
            <Box x={x} y={55} w={130} h={78} title={`${index + 1}. ${title}`} lines={index === 4 ? ["vector", "graph", "metadata"] : index === 5 ? ["claims", "citations", "gaps"] : []} tone={index === 5 ? "blue" : "light"} />
            {index < nodes.length - 1 ? <Arrow x1={x + 130} y1={94} x2={nodes[index + 1][0]} y2={94} /> : null}
          </g>
        ))}
        <path d="M835 145 C835 190, 410 195, 410 140" fill="none" stroke="#8a6d1f" strokeDasharray="5 5" markerEnd="url(#arrow)" />
      </svg>
    </Frame>
  );
}
...
export function IngestionWorkflowDiagram() {
  ...
  return (
    <Frame label="Document ingestion workflow" legend={legend}>
      <svg viewBox="0 0 940 280" className="min-w-[760px] w-full" role="img" aria-label="Document ingestion workflow">
        <ArrowDefs />
...
export function GovernanceWorkflowDiagram() {
  ...
  return (
    <Frame label="Governed knowledge lifecycle" legend={legend}>
      <svg viewBox="0 0 940 230" className="min-w-[760px] w-full" role="img" aria-label="Governed knowledge lifecycle">
        <ArrowDefs />
        ...
        <path d="M460 142 C460 195, 260 195, 260 142" fill="none" stroke="#9a463a" strokeDasharray="5 5" markerEnd="url(#arrow)" />
      </svg>
    </Frame>
  );
}
