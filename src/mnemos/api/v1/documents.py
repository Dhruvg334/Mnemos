...
from mnemos.schemas.upload import (
    EvidenceRegionResponse,
...
storage = S3Storage()
ingestion = IngestionLifecycle()


def safe_filename(value: str) -> str:
    value = re.sub(r"[^A-Za-z0-9._-]+", "_", value).strip("._")
    if not value:
        raise AppError("VALIDATION_ERROR", "Filename is not valid.", 422)
    return value[:180]
...
async def load_document(db: AsyncSession, document_id: str, principal: Principal) -> Document:
    ...
    require_site_access(principal, doc.site_id)
    return doc
...
async def create_upload_session(
    ...
    if payload.size_bytes > settings.max_upload_size_bytes:
        raise AppError(
            "FILE_TOO_LARGE",
            "The supplied file exceeds the configured upload limit.",
            413,
    ...
    duplicate = await db.scalar(
        select(Document).where(
            ...
            Document.sha256 == payload.sha256.lower(),
    ...
    session = UploadSession(document_id=doc.id, object_key=key, expires_at=expires)
    db.add(session)
    await db.flush()
    await storage.ensure_bucket()
    url = await storage.create_presigned_upload(
        key, payload.mime_type, settings.upload_session_expire_minutes * 60
    )
    await write_audit(
        db,
        organisation_id=site.organisation_id,
        ...
        metadata={"role": membership.role, "upload_session_id": session.id},
    )
    await db.commit()
    return Envelope(
...
async def confirm_upload(
    ...
    session = await db.scalar(
        select(UploadSession).where(
            UploadSession.id == payload.upload_session_id, UploadSession.document_id == doc.id
        )
    )
    if session is None:
        raise AppError("NOT_FOUND", "Upload session not found.", 404)
    if session.status != "pending":
        raise AppError("CONFLICT", "Upload session is not pending.", 409)
    if session.expires_at < datetime.now(UTC):
        session.status = "expired"
        await db.commit()
        raise AppError("CONFLICT", "Upload session has expired.", 409)
    if not await storage.object_exists(session.object_key):
        raise AppError(
            "DOCUMENT_NOT_READY", "The object has not been uploaded.", 409, retryable=True
        )
    ...
    if actual != doc.size_bytes:
        ...
        raise AppError(
            ...
            422,
            details={"declared_size_bytes": doc.size_bytes, "actual_size_bytes": actual},
        )
    doc.storage_key = session.object_key
    ...
    await write_audit(
        ...
        resource_id=doc.id,
        request_id=request.state.request_id,
        metadata={"upload_session_id": session.id, "processing_job_id": job.id},
    )
    await db.commit()
    await db.refresh(doc)
    return Envelope(
        data=DocumentResponse.model_validate(doc), meta=Meta(request_id=request.state.request_id)
    )
...
async def list_documents(
    ...
    return Envelope(
        data=[DocumentResponse.model_validate(x) for x in docs],
        meta=Meta(request_id=request.state.request_id),
    )
...
async def get_status(
    ...
    data = ProcessingStatusResponse(
        ...
        updated_at=(job.completed_at or job.started_at or job.created_at) if job else None,
    )
    return Envelope(data=data, meta=Meta(request_id=request.state.request_id))


@router.post(
    "/{document_id}/reprocess", response_model=Envelope[ProcessingStatusResponse], status_code=202
)
async def reprocess(
    document_id: str,
    request: Request,
...
@router.get(
    "/{document_id}/evidence/{evidence_region_id}", response_model=Envelope[EvidenceRegionResponse]
)
async def get_evidence(
    ...
    doc = await load_document(db, document_id, principal)
    ev = await db.scalar(
        select(EvidenceRegion).where(
            EvidenceRegion.id == evidence_region_id, EvidenceRegion.document_id == doc.id
        )
    )
    if ev is None:
        raise AppError("NOT_FOUND", "Evidence region not found.", 404)
    data = EvidenceRegionResponse(
        id=ev.id,
        document_id=ev.document_id,
        page_or_sheet=ev.page_or_sheet,
        locator=ev.locator,
