...
from mnemos.schemas.knowledge import (
    KnowledgeCardCreate,
...
async def _get_card(db: AsyncSession, card_id: str, principal: Principal) -> KnowledgeCard:
    ...
    if card is None:
        raise AppError("NOT_FOUND", "Knowledge card not found.", 404)
...
async def create_card(
    ...
    if payload.asset_id:
        ...
        if asset is None or asset.site_id != site.id:
    ...
    if payload.supersedes_id:
        ...
        if old is None or old.site_id != site.id or old.status != "approved":
            raise AppError(
                "VALIDATION_ERROR", "Only an approved card in the same site can be superseded.", 422
            )
    card = KnowledgeCard(
    ...
    return Envelope(
        data=KnowledgeCardResponse.model_validate(card),
    ...
    rows = list(
        (
            await db.scalars(
                ...
                .where(KnowledgeCard.site_id == site_id)
                .order_by(KnowledgeCard.updated_at.desc())
            )
        ).all()
    )
    return Envelope(
        data=[KnowledgeCardResponse.model_validate(x) for x in rows],
        meta=Meta(request_id=request.state.request_id),
    )
...
async def update_card(
    ...
    card = await _get_card(db, card_id, principal)
    require_site_role(principal, card.site_id, WRITE_ROLES)
    if card.status != "draft":
        raise AppError("CONFLICT", "Only draft knowledge cards can be edited.", 409)
    if card.author_id != principal.user.id and not any(
        m.role in {"platform_admin", "organisation_admin", "site_admin"}
        for m in principal.memberships
    ):
        raise AppError("FORBIDDEN", "Only the author or an administrator can edit this card.", 403)
...
async def submit_card(
    ...
    require_site_role(principal, card.site_id, WRITE_ROLES)
    if card.status != "draft":
        raise AppError("CONFLICT", "Only draft knowledge cards can be submitted.", 409)
    card.status = "pending_review"
    card.submitted_at = datetime.now(UTC)
    await db.commit()
    await db.refresh(card)
    return Envelope(
        data=KnowledgeCardResponse.model_validate(card),
...
async def approve_card(
    ...
    card = await _get_card(db, card_id, principal)
    require_site_role(principal, card.site_id, REVIEW_ROLES)
    if card.status != "pending_review":
        raise AppError("CONFLICT", "Knowledge card is not awaiting review.", 409)
    if card.author_id == principal.user.id:
        raise AppError("FORBIDDEN", "Authors cannot approve their own knowledge cards.", 403)
    card.status = "approved"
    card.reviewer_id = principal.user.id
    card.review_note = payload.note
...
async def reject_card(
    ...
    if card.status != "pending_review":
        raise AppError("CONFLICT", "Knowledge card is not awaiting review.", 409)
    if card.author_id == principal.user.id:
        raise AppError("FORBIDDEN", "Authors cannot review their own knowledge cards.", 403)
