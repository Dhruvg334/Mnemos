...
from mnemos.schemas.rca import (
    ...
    RCAObservationCreate,
    RCAObservationResponse,
    RCAResponse,
    RCAReviewRequest,
    RCAUpdate,
)
...
async def create_rca(
    ...
    await db.commit()
    return Envelope(
        data=_serialize(await load_rca_bundle(db, rca)),
...
@router.post(
    "/{rca_id}/observations", response_model=Envelope[RCAObservationResponse], status_code=201
)
async def add_observation(
    rca_id: str,
    payload: RCAObservationCreate,
    request: Request,
    principal: Principal = Depends(get_principal),
    db: AsyncSession = Depends(get_db),
):
    ...
    require_site_role(principal, rca.site_id, WRITE_ROLES)
    if rca.status != "draft":
        raise AppError("CONFLICT", "Only draft RCAs can be changed.", 409)
    row = RCAObservation(rca_id=rca.id, **payload.model_dump())
    db.add(row)
    await db.commit()
    await db.refresh(row)
    return Envelope(
        data=RCAObservationResponse.model_validate(row),
        meta=Meta(request_id=request.state.request_id),
    )


@router.post(
    "/{rca_id}/hypotheses", response_model=Envelope[RCAHypothesisResponse], status_code=201
)
async def add_hypothesis(
    rca_id: str,
    payload: RCAHypothesisCreate,
    request: Request,
    principal: Principal = Depends(get_principal),
    db: AsyncSession = Depends(get_db),
):
    ...
    if rca.status != "draft":
        raise AppError("CONFLICT", "Only draft RCAs can be changed.", 409)
    row = RCAHypothesis(rca_id=rca.id, **payload.model_dump())
    db.add(row)
    await db.commit()
    await db.refresh(row)
    return Envelope(
        data=RCAHypothesisResponse.model_validate(row),
        meta=Meta(request_id=request.state.request_id),
    )
...
async def add_action(
    ...
    if rca.status != "draft":
        raise AppError("CONFLICT", "Only draft RCAs can be changed.", 409)
    row = RCAAction(rca_id=rca.id, **payload.model_dump())
    db.add(row)
    await db.commit()
    await db.refresh(row)
    return Envelope(
        data=RCAActionResponse.model_validate(row), meta=Meta(request_id=request.state.request_id)
    )
...
async def submit_rca(
    ...
    require_site_role(principal, rca.site_id, WRITE_ROLES)
    if rca.status != "draft":
        raise AppError("CONFLICT", "Only draft RCAs can be submitted.", 409)
    bundle = await load_rca_bundle(db, rca)
    if not bundle["observations"] or not bundle["hypotheses"]:
        raise AppError(
            "VALIDATION_ERROR", "RCA requires at least one observation and one hypothesis.", 422
        )
    rca.status = "under_review"
    rca.submitted_by = principal.user.id
    rca.submitted_at = datetime.now(UTC)
    await db.commit()
    return Envelope(
        data=_serialize(await load_rca_bundle(db, rca)),
        meta=Meta(request_id=request.state.request_id),
...
async def approve_rca(
    rca_id: str,
    payload: RCAReviewRequest,
    request: Request,
    principal: Principal = Depends(get_principal),
    db: AsyncSession = Depends(get_db),
):
    rca = await _get_rca(db, rca_id, principal)
    require_site_role(principal, rca.site_id, APPROVE_ROLES)
    if rca.status != "under_review":
        raise AppError("CONFLICT", "RCA is not awaiting review.", 409)
    ...
    bundle = await load_rca_bundle(db, rca)
    rca.status = "approved"
    rca.approved_by = principal.user.id
    rca.approved_at = datetime.now(UTC)
    rca.review_note = payload.note
    rca.approved_snapshot = rca_snapshot(bundle)
    await db.commit()
    return Envelope(
        data=_serialize(await load_rca_bundle(db, rca)),
        meta=Meta(request_id=request.state.request_id),
...
async def reject_rca(
    ...
    rca.status = "rejected"
    rca.rejected_by = principal.user.id
    rca.review_note = payload.note
    await db.commit()
    return Envelope(
        data=_serialize(await load_rca_bundle(db, rca)),
