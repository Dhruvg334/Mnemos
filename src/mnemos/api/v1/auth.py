...
from mnemos.schemas.auth import (
    DevLoginRequest, LoginRequest, LogoutRequest, PasswordChangeRequest,
...
async def login(payload: LoginRequest, request: Request, db: AsyncSession = Depends(get_db)):
    await enforce_public_rate_limit(
        request, _client_identity(request),
    ...
    if user.locked_until and user.locked_until > now:
        raise AppError("ACCOUNT_LOCKED", "Account is temporarily locked. Try again later.", 423, retryable=True)
...
async def dev_login(payload: DevLoginRequest, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    user = await db.scalar(select(User).where(
        User.email == str(payload.email).lower(), User.is_active.is_(True)
    ))
    if user is None:
...
async def change_password(payload: PasswordChangeRequest, request: Request,
    ...
    if verify_password(payload.new_password, user.password_hash):
        raise AppError("VALIDATION_ERROR", "New password must differ from the current password.", 422)
    user.password_hash = hash_password(payload.new_password)
    user.token_version += 1
    now = datetime.now(UTC)
    tokens = list((await db.scalars(select(RefreshToken).where(
...
async def me(request: Request, principal: Principal = Depends(get_principal)):
    return Envelope(data=UserResponse.model_validate(principal.user),
                    meta=Meta(request_id=request.state.request_id))
