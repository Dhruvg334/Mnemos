...
class AgentResponse(BaseModel):
    ...
    graph_paths: list[list[str]] = Field(
        default_factory=list, description="KG paths supporting the answer"
    )
    metadata: dict[str, Any] = Field(default_factory=dict)
...
class Hypothesis(BaseModel):
    ...
    text: str
    support_status: str = (
        "not_evaluated"  # not_evaluated, supported, refuted, partially_supported, uncertain
    )
    confidence_score: float = Field(default=0.0, ge=0.0, le=1.0)
    ...
    causal_chain: list[str] = Field(
        default_factory=list, description="Ordered chain of cause→effect links"
    )
...
class KnowledgeSubmission(BaseModel):
    ...
    conflicts_with: list[str] = Field(
        default_factory=list, description="IDs of conflicting submissions"
    )
...
class HistoricalComparison(BaseModel):
    ...
    applicable_actions: list[str] = Field(
        default_factory=list, description="Actions from history worth applying"
    )
    reasoning: str = ""
