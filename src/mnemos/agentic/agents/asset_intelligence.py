...
class AssetIntelligenceAgent:
    ...
    async def process(self, state: AgentState) -> dict[str, Any]:
        ...
        evidence: list[EvidenceSource] = state.get("evidence_bundle", {}).get(
            "verified_evidence", []
        )
        ...
        for entity in entities:
            ...
            asset_evidence = [
                s
                for s in evidence
                if s.provenance.node_id == entity.entity_id or not s.provenance.node_id
            ]
            health_score = (
                sum(s.confidence_score for s in asset_evidence) / len(asset_evidence)
                if asset_evidence
                else 0.0
            )
