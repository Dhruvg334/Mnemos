...
class IndustrialEvaluator:
    ...
    async def evaluate_sample(self, sample: EvalSample, result: SampleResult) -> List[MetricResult]:
        ...
        if sample.expected_document_ids:
            ...
            precision = len(expected_docs.intersection(actual_docs)) / len(actual_docs) if actual_docs else 0.0
            # Recall: expected docs that were cited / total expected docs
            recall = len(expected_docs.intersection(actual_docs)) / len(expected_docs) if expected_docs else 1.0
        ...
        if sample.expected_document_ids:
            ...
            retrieval_recall = len(expected_retrieved.intersection(actual_retrieved)) / len(expected_retrieved)
