...
class GraphTraversalInput(BaseModel):
    ...
    start_node_id: str = Field(..., description="Starting node ID (asset, component, incident, etc.).")
    ...
    relationship_types: list[str] = Field(default_factory=list, description="Filter by relationship types.")
...
class DocumentRetrievalInput(BaseModel):
    ...
    page_range: tuple[int, int] | None = Field(None, description="Optional page range (start, end).")
...
class SimilarFailuresInput(BaseModel):
    ...
    max_results: int = Field(default=10, ge=1, le=50, description="Maximum similar failures to return.")
...
class RevisionCheckInput(BaseModel):
    ...
    expected_version: int | None = Field(None, description="Expected version (None = check latest).")
...
class ActionCreationInput(BaseModel):
    ...
    action_type: str = Field(..., description="Action type: TEST, INSPECTION, REPAIR, MONITOR, PROCEDURE_UPDATE, TRAINING.")
