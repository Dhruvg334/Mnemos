...
class S3Storage:
    ...
    async def ensure_bucket(self) -> None:
        def work() -> None:
            ...
            except ClientError: self.client.create_bucket(Bucket=self.bucket)
        await asyncio.to_thread(work)

    async def create_presigned_upload(self, object_key: str, mime_type: str, expires_seconds: int) -> str:
        return await asyncio.to_thread(
            self.client.generate_presigned_url, "put_object",
    ...
    async def object_exists(self, object_key: str) -> bool:
        def work() -> bool:
            try:
                self.client.head_object(Bucket=self.bucket, Key=object_key); return True
