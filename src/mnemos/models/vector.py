...
class ChunkEmbedding(Base):
    ...
    __table_args__ = (
        Index("ix_chunk_embedding_vector", "embedding", postgresql_using="hnsw", postgresql_with={"m": 16, "ef_construction": 64}, postgresql_ops={"embedding": "vector_cosine_ops"}),
    )
...
class GraphNodeMapping(Base):
    ...
    evidence_region_id: Mapped[str | None] = mapped_column(ForeignKey("evidence_regions.id", ondelete="CASCADE"), index=True, nullable=True)
    chunk_id: Mapped[str | None] = mapped_column(ForeignKey("document_chunks.id", ondelete="CASCADE"), index=True, nullable=True)
